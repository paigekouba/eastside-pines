...
# no gaps for now    # geom_sf(data=results[[i]], col="white", linewidth = 0.7, fill= "purple", alpha=0.3) +
          ...
          labs( #title = paste("Living Trees at", names[i]),
               ...
               size = 10) +
               # size = "Crown (m)") +
          theme_void() + theme(legend.text = element_text(size=18), legend.title = element_text(size=18)) #+ theme(legend.position = "none")
...
library(ggpubr)
ggarrange(ICO_maps[[2]], ICO_maps[[13]], ICO_maps[[1]], ICO_maps[[4]], ICO_maps[[14]], ICO_maps[[3]], ICO_maps[[6]], ICO_maps[[15]], ICO_maps[[5]], ncol=3, nrow=3, common.legend = T, legend.grob = get_legend(ICO_maps[[13]]), legend = "right") # IS
