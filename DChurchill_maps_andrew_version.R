...
Plot.pp.cluster = function(point.data,tree.data,species.color="Eastside",radi = 3,cm="Y",axes="n",QM=F,newPlot=T,
  ...
  #y.min = point.data$window$yrange[1] #EDIT: y.min and yrange ? yes
  x.max = 60 
  y.max = 60
  x.min = -60
  y.min = -60
...
#    if(newPlot == T) plot(point.data$x,point.data$y,cex=.1,pch=1,xlab="",ylab="",xaxt#=axes,yaxt=axes,ylim=c(-60,60),xlim=c(-60,60)) 
  ...
  for (i in 1:n.pts) draw.circle2(point.data$x[i], point.data$y[i], radius=(100, 0, 3), nv=100, col="darkgreen", lty=0, lwd=1, density=NULL, angle=NULL)
...
Plot.pp.cluster(point.data, tree.data,species.color="Eastside", radi = 1, cm="Y", axes="n", QM=F, newPlot=T, clust.breaks=c(1,2,5,10,16),d.factor=120)
